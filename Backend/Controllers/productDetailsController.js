  ...
  Products.findById(req.params.Id)
    .then((product) => {
      if (!product) {
        res.json({
          status: "Fail",
          message: "Product data not found with id!",
          err: err,
        });
      }
      res.json({
        status: "Success",
        message: "Product data found with id!!!",
        data: product,
      });
    })
    .catch((err) => {
  ...
  Products.findByIdAndDelete(
    ...
    function (err, result) {
      if (err) {
        res.json({
          status: "Fail",
          message: "Product not deleted with id!",
          err: err,
        });
      }
      res.json({
...
exports.getCategoryByName = (req, res) => {
  Products.find({
    ...
    .then((category) => {
      if (!category) {
        res.json({ status: "fail", message: "fail to find category by name!" });
      }
      res.json({
        status: "success",
