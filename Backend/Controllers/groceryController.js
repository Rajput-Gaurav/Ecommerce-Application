...
exports.create = (req, res) => {
    ...
    Grocery.create(grocery, function(err, result) {
            if(err)
                ...
                res.send({ status: "success", message: "Grocery added successfully!", data: result});            
        });
}
    ...
    Grocery.find( {isDeleted: false})
        .then(grocery => {
            if(!grocery) {
                ...
                res.json({ status: "Success", message: "Grocery found successfully!!!", data: grocery});
...
exports.findById = (req, res) => {
    if(!req.params.Id) {
        return res.send({
            status: "Fail", message: "Grocery id not found!" + req.params.Id
        });
    }
    Grocery.findById(req.params.Id)
        .then(grocery => {
            if(!grocery) {
                res.json({
                    status: "Fail",
                    message: "Grocery data not found with id!", err: err
                });
            }
                res.json({
                    status: "Success",
                    message: "Grocery data found with id!!!", data: grocery
                });
        })
        .catch(err => {
            res.json({
...
exports.updateGroceryStatus = (req, res) => {
    if (!req.params.Id) {
        ...
        Grocery.findByIdAndUpdate(req.params.Id, { $set: { status: req.body.status}}, { new: false}, function(err, result){
            if (err) {
                res.json({ status: "error", message: "Status not update!", err: err});
            }
                res.json({ status: "Success", message: "Grocery status is updated successfully!!!", data: result});
        });
    }
};
...
exports.update = (req, res) => {
    ...
    Grocery.findByIdAndUpdate(req.params.Id, { $set: req.body}, {new: false}, function(err, result){
        if (err) {
            res.json({ status: "error", message: "Grocery not updated!", err: err});
        }
            res.json({ status: "Success", message: "Grocery updated successfully!!!", data: result});
    });
};
...
exports.delete = (req, res) => {
    ...
    Grocery.findByIdAndDelete(req.params.Id, { new: false}, function(err, result){
        if(err) {
            res.json({
                status: "Fail",
                message: "Grocery not deleted with id!", err: err
            });
        }
        res.json({ 
            status: "Success",
